...
description: "Activate when working with Filament admin panels, Resources, Forms, Tables, Actions, Notifications, or Widgets. Covers panel setup, scaffolding, common patterns, and Pest testing for Filament v5. Do not activate for plain Laravel or Livewire tasks unrelated to Filament."
license: MIT
metadata:
  author: filamentphp
---
...
## Documentation
Use `search-docs` to look up specific Filament v5 APIs and configuration options.

## Overview
Filament v5 is a server-driven UI toolkit for Laravel built on Livewire v4, Alpine.js, and Tailwind CSS v4. It requires PHP 8.2+, Laravel v11.28+, and Livewire v4.0+. All components use a fluent, static `make()` API - never instantiate them with `new`. The `filament/schemas` package is a new first-class package in v5 that underpins forms, infolists, and layout components.

## Scaffolding
Always generate Filament files through Artisan, never manually:

- `{{ $assist->artisanCommand('make:filament-resource Post --generate') }}` - Resource with schema, table, and CRUD pages
...
## Resources
Resources are static classes living in `app/Filament/Resources`. In v5, the `form()` method is backed by the new `filament/schemas` package - use the same fluent `->schema([...])` API:
...
## Testing
All Filament tests require authentication. Use `livewire()` to drive Filament pages - do not call page URLs directly. In v5, use `assertSchemaExists()` instead of the deprecated `assertFormExists()`:
...
## Upgrade from v3 / v4
Run the automated upgrade script before any manual changes:

@boostsnippet("Upgrade script", "bash")
...
- Using `assertFormExists()` - it is deprecated in v5, use `assertSchemaExists()` instead
- Targeting v3 or v4 branches for new features - only v5.x accepts new features
